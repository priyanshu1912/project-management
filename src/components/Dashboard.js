...
import { dashboardData, completedTasks,users } from "../Mock-data.js";
...
let initial = {
  ...
  function Dashboard() {
    ...
    let currentProject = users[0].projects[0];

    return (
      <>
        <div className="py-3 px-3">
          <div className="font-bold">
            {currentProject.project_name.toUpperCase()} - {currentUser.name}
          </div>
          <div className="text-sm bg-white w-full min-h-screen flex sm:flex-col gap-3 mt-2">
            ...
            <div className="w-9/12 sm:w-full flex gap-2 rounded-md overflow-auto scroll-smooth hide-scrollbar">
              {currentProject.task_lists.map((item, index) => {
                return (
                  <div
                    key={index}
